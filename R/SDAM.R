...
#'
#' Estimate high-dimensional additive models using spectral deconfounding.
...
SDAM <- function(X, Y, Q_type = "trim", trim_quantile = 0.5, q_hat = 0, cv_k = 10, cv_method = "1se", n_K = 4, n_lambda1 = 15, n_lambda2 = 30, Q_scale = TRUE, ind_lin = NULL, mc.cores = 1){
  ...
  Qf <- get_Qf(X, type = Q_type, trim_quantile = trim_quantile, q_hat = q_hat, gpu = FALSE, scaling = Q_scale)
  QY <- Qf(Y)
  ...
  mse_fold <- function(l){
    # MSEl <- lapply(X = lmodK, FUN = mse_fold_K, l = l)
